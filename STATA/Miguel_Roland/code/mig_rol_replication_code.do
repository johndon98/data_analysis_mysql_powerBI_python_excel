*Replication - Miguel Roland

cd "C:\Users\johnd\OneDrive\Desktop\sose21\Dev\Miguel_roland"

log using ".\data\miguel_roland_rep", replace
set more off

*load data
use ".\data\MR2011_replication.dta", clear
...
*(iv)
*I think total ordnance and total km2 (>or< 17°) used to calculate ordnancekm2 separately.
...
outreg2 using ".\output\reg1.xlsx", replace depvar
*ii)
...
outreg2 using ".\output\reg2.xlsx", replace title(dependent variable: estimated poverty rate 1999) ctitle(ex-south)
reg poverty_p0 ordnancekm2 if south!=1, robust
outreg2 using ".\output\reg2.xlsx", ctitle(ex-north)
reg poverty_p0 ordnancekm2 if popdensity6061<200, robust
outreg2 using ".\output\reg2.xlsx", ctitle(rural)
reg poverty_p0 ordnancekm2 if popdensity6061>200, robust
outreg2 using ".\output\reg2.xlsx", ctitle(urban)
reg poverty_p0 ordnancekm2 c.ordnancekm2#c.ordnancekm2, robust
outreg2 using ".\output\reg2.xlsx", ctitle(square bombing)
sort ordnancekm2
...
outreg2 using ".\output\reg2.xlsx", ctitle(top10percent)
...
outreg2 using ".\output\reg3.xlsx", replace nocons dec(4) drop(soil*) addtext(District soil controls, YES) ctitle(OLS)
...
outreg2 using ".\output\first_stage.xlsx", replace dec(4) nocons drop(soil*) addtext(District soil controls, YES)
