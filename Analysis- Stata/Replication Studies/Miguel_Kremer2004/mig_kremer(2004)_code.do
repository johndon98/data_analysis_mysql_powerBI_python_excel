...
log using migkrem_2004_rep, replace
set more off 

cd "C:\Users\johnd\OneDrive\Desktop\miguel_kremer(2004)"
...
foreach i in sex elg98 stdgap yrbirth {
	...
	outreg2 using table1_panelA.xls, dec(2) `replace'
	local replace
}
...
foreach i in days_present bloodst_98_58 sick_often malaria_98_48 child_clean {
	...
	outreg2 using table1_panelB.xls, dec(2) `replace'
	local replace
}
restore
...
foreach i in distlake pup_pop latr_pup z_inf98 pop1_3km_updated pop1_6km_updated popT_3km_updated popT_6km_updated {
	...
	outreg2 using table1_panelC.xls, dec(2) `replace'
	local replace
}

*Table 3

use comply.dta, clear
...
merge 1:1 pupid using comply_new
...
outreg2 using table7.xls, dec(2) replace
...
saveold table7_repdata, replace
...
merge 1:m schid using table7_repdata
